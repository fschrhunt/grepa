...
impl McpClient {
    ...
    pub fn new(timeout_seconds: u64) -> Result<Self, String> {
        let config = ureq::Agent::config_builder()
            ...
            .max_redirects(0)
            .user_agent(concat!("grep-app-lite/", env!("CARGO_PKG_VERSION")))
    ...
    pub fn search(&mut self, options: &Options) -> Result<Vec<String>, String> {
        let initialize = json!({
            ...
            "params": {"protocolVersion": PROTOCOL_VERSION, "capabilities": {}, "clientInfo": {"name": "grep-app-lite", "version": env!("CARGO_PKG_VERSION")}}
        });
